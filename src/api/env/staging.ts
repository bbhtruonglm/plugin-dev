/**
 * Vite host for production
 */
export const VITE_APP_BE_HOST = 'https://chatbox-public-v2.botbanhang.vn'
/** export const VITE_APP_SOCKET_HOST = 'wss://dev-api.botbanhang.vn/socket' */
export const VITE_APP_SOCKET_HOST =
  'wss://chatbox-public-v2.botbanhang.vn/socket'
/** IMG HOST - OLD */
export const VITE_IMAGE_HOST = 'https://chatbox-static-v3.botbanhang.vn'
/** DOMAIN */
export const DOMAIN = 'https://retion.ai'
/** CND URL */
export const VITE_CDN = 'https://cdn.botbanhang.vn'
/** ID WIDGET */
export const ID_WIDGET = '00de4446885a43c5b58ef16dba0f5058'

export const DOMAIN_TRIGGER_BTN =
  'https://chatbot-api.botbanhang.vn/public/json/chatbot_website'
/** API WIDGET */
export const APP_URL = ''
export const WIDGET_URL = ''
export const CHATBOT_URL = ''
export const APP_V2_URL = ''
