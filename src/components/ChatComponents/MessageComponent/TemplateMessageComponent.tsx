...
import FileAttachment from './components/QuickPreview/FileAttachment'
import GenericTemplate from './components/QuickPreview/GenericTemplate'
// Các component nhỏ đã tách ra
...
function TemplateMessageComponent({ data }: MessageProps) {
  ...
  return (
    <div className="flex flex-col rounded-lg group relative w-full h-full justify-center items-center">
      ...
      {data?.message_attachments?.length === 1 &&
        data?.message_attachments?.[0]?.type === 'image' && (
          <ImageAttachment data={data} />
        )}

      {/* MULTIPLE IMAGES */}
      {data?.message_attachments?.length > 1 &&
        data?.message_attachments?.[0]?.type === 'image' && (
          <MultipleImageGallery data={data} />
        )}

      {/* VIDEO */}
      {data?.message_attachments?.[0]?.type === 'video' && (
        <VideoAttachment data={data} />
      )}

      {/* AUDIO */}
      {data?.message_attachments?.[0]?.type === 'audio' && (
        <AudioAttachment data={data} />
      )}
      ...
      {data?.message_text &&
        data?.message_type !== 'system' &&
        data?.message_attachments?.[0]?.type === 'fallback' && (
          <FallbackLink data={data} />
        )}

      {/* BUTTON TEMPLATE */}
      {data?.message_attachments?.[0]?.type === 'template' &&
        data?.message_attachments?.[0]?.payload?.template_type === 'button' && (
          <ButtonTemplate data={data} />
        )}

      {/* GENERIC TEMPLATE */}
      {data?.message_attachments?.[0]?.type === 'template' &&
        data?.message_attachments?.[0]?.payload?.template_type ===
          'generic' && <GenericTemplate data={data} />}
    </div>
  )
}
