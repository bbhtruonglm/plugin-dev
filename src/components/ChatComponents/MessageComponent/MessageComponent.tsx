...
import MessageButtonTemplate from './components/MessageButtonTemplate'
import MessageFallback from './components/MessageFallback'
import MessageFile from './components/MessageFile'
import MessageGenericTemplate from './components/MessageGenericTemplate'
...
const MessageComponent = React.memo(({ data }: MessageProps) => {
  ...
  const containerClass = getMessageClasses(messageType, AI_STATUS)

  return (
    <div
      className={`group relative flex flex-col transition-all duration-300 ease-out gap-y-4 rounded-lg ${containerClass}`}
      // className={`flex flex-col transition-all duration-300 ease-out gap-y-4 rounded-lg group relative ${getMessageClasses(
      //   data?.message_type
      // )}`}
    >
      {(messageType === 'page' || messageType === 'client') && (
        <div
          className={`absolute w-36 bottom-full ${
            data?.message_type === 'page' ? 'left-0' : 'right-0'
          }  text-xs font-semibold text-slate-700 bg-transparent rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
